(function(root, factory) {
    ...
    function State(rule, dot, reference, wantedBy, column) {
        this.rule = rule;
        this.dot = dot;
    ...
    State.prototype.nextState = function(child, column) {
        var state = new State(this.rule, this.dot + 1, this.reference, this.wantedBy, column);
        state.left = this;
        state.right = child;
        if (state.isComplete) {
    ...
    {
        return this.nextState(right, this.column + right.column - right.reference);
    }
    ...
    Column.prototype.handleWants = function*(state, exp)
    {
        ...
        let newone = this.wants[exp].wants.size === 0;
        yield* this.wants[exp].addWant(state);
        if (newone) {
    ...
    Parser.prototype.dialog = function*(g) {
        ...
        while (state = yield) {
            while (!this.table[state.column]) {
                this.table.push(new Column(this.grammar, this.table.length));
            }
            let column = this.table[state.column];
    ...
    Parser.prototype.socrates = function* (g) {
        ...
        while (p = d.next(r)) {
            ...
            if (v === undefined) {
                ...
                iter++;
                //if (r)
                //    console.log("@"+r.column+"/" + g.length + " " + q.length + " in: " + r);
            } else {
                //console.log("@"+v.column+"/" + g.length + " out: " + v);
                if (!maxv || v.column > maxv.column) {
                    maxv = v;
                }
                if (v.column == g.length && v.isComplete && v.rule.name == this.grammar.start) {
                    v.finish();
                    yield v.data;
                    succ++;
                } else {
